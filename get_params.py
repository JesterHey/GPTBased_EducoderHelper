...
def get_parameters(url: str, user_name: str, password: str,retry:int=2):
    '''
    用于获得实训作业的参数
    url:实训网址
    user_name:用户名
    ...
    while retry <= 2:
        ...
        if is_practice(url):
            ...
            if exist:
                ...
                # 展开后，服务器会返回一个challenges.json文件，里面包含了所有关卡的参数
                ...
                try:
                    while i <= task_num:
                        ...
                        shixun_id = response.json()['challenge']['shixun_id']
                        print(f'第{i}关的id为{challenge_id},爬取成功')
                        #获取任务描述(如果存在的话)
                        time.sleep(2)
                ...
                if len(total) != task_num:
                    if retry <= 2:
                        print('参数有数量不对等，正在重新爬取')
                        retry -= 1
                        continue
                    else :
                        print('参数爬取失败，超过最大重试次数，爬取失败')
                        return
                elif len(total) == task_num:
                    for i,j in total.items():
                        if j['code'] == '' and retry <= 2:
                            print('参数有空值，正在重新爬取')
                            retry -= 1
                            continue
